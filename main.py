...
for index, row in df.iterrows():
	...
	if row['numberOfCodedValues'] != 367236:
		error_comleteness.append(row['filename'])

	print ('{} row passed'.format(index))

#Проверка заблоговременности
for i in range(number_of_rows-1):
	if (
		df.loc[i,'P1']+1 != df.loc[i+1,'P1'] and 
		df.loc[i,'dataDate'] == df.loc[i+1,'dataDate'] and 
		df.loc[i, 'dataTime'] == df.loc[i+1,'dataTime']
		):
...
zero_files = cp.completeness() #возращает файлы, в которых не 3 записи
error_comleteness.extend(zero_files)
